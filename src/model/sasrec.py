...
from model._modules_wave import WavspaEncoder
...
class SASRecModel(SequentialRecModel):
    ...
    def get_attentions(self, input_ids, user_ids=None):
        """
        Returns:
            all_attentions: list of length num_layers, each is Tensor [B, H, L, L]
        """
        extended_attention_mask = self.get_attention_mask(input_ids)
        sequence_emb = self.add_position_embedding(input_ids)
